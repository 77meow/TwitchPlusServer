...
public class MySQLConnection {
    ...
    public void setFavoriteItem(String userId, Item item) throws MySQLException {
        ...
        // INSERT USERID and ITEMID to favorite table, maybe insert item into item table?
        // 你添加的东西可能别人没添加过
        // 取决于添加的item之前存不存在
        // Drawback: 有人胡乱输入一个userID like 1111 DROP TABLES / 1111 OR 1=1
        // String template = "INSERT INTO favorite_records (user_id, item_id) VALUES (%s, %s)";
        // String sqp = String.format(template, userId, item.getId());

        // 以下这种方法更安全
        // If duplicates -> ignore instead of throw exception
        saveItem(item);
        ...
        try {
            ...
            e.printStackTrace();
            throw new MySQLException("Failed to save favorite to Database");
        }
    }
    ...
    public void unsetFavoriteItem(String userId, String itemId) throws MySQLException {
        ...
        // Maybe remove item in item table??
        // If someone else also fav this item, we cannot remove the item
        // If you are the only person fav this item, you can go either way
        //  -> DELETE delete immediately
        //  -> KEEP 把所有要删的东西攒起来最后一起删
    ...
    // If user has fav some items, then we dont recommend them what they've fav
    public Set<String> getFavoriteItemIds(String userId) throws MySQLException {
        ...
        try {
            statement = conn.prepareStatement(sql);
            statement.setString(1, userId);
            // default reference: -1 index; iterator
            ...
            e.printStackTrace();
            throw new MySQLException("Failed to select favorite item to Database");
        }
